...
def minMaxHelper(gameState, turn, depth, alpha, beta):
	if depth <= 0:
	...
	if turn:
		for state in possibleStates:
			alpha = rating = minMaxHelper(state, not turn, depth - 1, alpha, beta)
		return max([minMaxHelper(state, not turn, depth - 1) for state in possibleStates])
	else:
		for state in possibleStates:
			beta = rating = minMaxHelper(state, not turn, depth - 1, alpha, beta)
